...
class modCarouselckHelper {
	...
	static function getArticle(&$item, $params) {
		...
		$item->article->text = self::truncate($item->article->text, $params->get('articlelength', '150'), true, false);
...
//			}
			$item->displayIntrotext = $show_introtext ? self::truncate($item->introtext, $introtext_limit, true, false) : '';
			...
			if (isset($item->images['image_intro']) && $item->images['image_intro']
				...
				$slideItem->article->text = self::truncate($slideItem->article->text, $params->get('articlelength', '150'), true, false);
