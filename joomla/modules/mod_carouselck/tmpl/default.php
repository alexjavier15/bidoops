...
<div class="carouselck<?php echo $params->get('moduleclass_sfx'); ?> carouselck_wrap <?php echo $params->get('skin'); ?>" id="carouselck_wrap_<?php echo $module->id; ?>"<?php echo $width; ?>>
	...
	foreach ($items as $item) {
		...
		$dataalbum = ($params->get('lightboxgroupalbum', '0')) ? '[albumcarouselck' .$module->id .']' : '';
		$datarel = ($imgtarget == 'lightbox') ? 'data-rel="lightbox' . $dataalbum . '" ' : '';
		if ($params->get('articlelink', 'readmore') == 'image' && isset($item->article->link) && $item->article->link) {
			$item->imglink = $item->article->link;
		} else if ($params->get('lightboxautolinkimages', '0') == '1') {
			$item->imglink = $item->imgname;
		}
		?>
		<div class="carouselck" <?php echo $datarel . $datatitle; ?> data-thumb="<?php echo $item->imgthumb; ?>" data-src="<?php echo $item->imgname; ?>" <?php if ($item->imglink) echo 'data-link="' . $item->imglink . '" data-target="' . $imgtarget . '"'; ?>>
			...
			// (($params->get('lightboxcaption', 'caption') != 'title' || $imgtarget != 'lightbox') || !$item->imglink)
