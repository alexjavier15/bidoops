...
class modB4CarouselHelper
{
    static function getImagesFromFolder(&$params) {
		...
		$target = modB4CarouselHelper::getSlideTarget($params->get('link'));
	...
	static function getImagesFromDJImageSlider(&$params) {
		...
		foreach($slides as $slide){
			...
			$slide->link = modB4CarouselHelper::getSlideLink($slide);
			$slide->description = modB4CarouselHelper::getSlideDescription($slide, $params->get('limit_desc'));
			...
			if(empty($slide->target)) $slide->target = modB4CarouselHelper::getSlideTarget($slide->link);
		}
	...
	static function getAnimationOptions(&$params) {
		...
		$css3transition = $params->get('css3') ? modB4CarouselHelper::getCSS3Transition($transition, $easing) : '';
		...
		if (version_compare(JVERSION, '3.0.0', '<')) { // Joomla!2.5 - Mootools
			...
			$transition = modB4CarouselHelper::getMooTransition($transition);
	...
	static function getNavigation(&$params, &$mid) {
		...
		if($params->get('slider_type')==1) {			
			if(empty($prev) || !file_exists(JPATH_ROOT.DS.$prev)) $prev = 'modules/mod_b4_carousel/themes/'.$theme.'/images/up.png';			
			if(empty($next) || !file_exists(JPATH_ROOT.DS.$next)) $next = 'modules/mod_b4_carousel/themes/'.$theme.'/images/down.png';
		} else {			
			if(empty($prev) || !file_exists(JPATH_ROOT.DS.$prev)) $prev = 'modules/mod_b4_carousel/themes/'.$theme.'/images/prev.png';			
			if(empty($next) || !file_exists(JPATH_ROOT.DS.$next)) $next = 'modules/mod_b4_carousel/themes/'.$theme.'/images/next.png';
		}
		if(empty($play) || !file_exists(JPATH_ROOT.DS.$play)) $play = 'modules/mod_b4_carousel/themes/'.$theme.'/images/play.png';
		if(empty($pause) || !file_exists(JPATH_ROOT.DS.$pause)) $pause = 'modules/mod_b4_carousel/themes/'.$theme.'/images/pause.png';
