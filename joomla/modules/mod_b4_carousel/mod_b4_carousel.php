<?php
/**
 * @version $Id: MOD_B4_CAROUSEL.php 48 2017-08-04 11:41:27Z szymon $
...
if($params->get('slider_source')==1) {
	...
	if(!JComponentHelper::isEnabled('com_djimageslider', true)){
		$app->enqueueMessage(JText::_('MOD_B4_CAROUSEL_NO_COMPONENT'),'notice');
		return;
	}
	$slides = modB4CarouselHelper::getImagesFromDJImageSlider($params);
	if($slides==null) {
		$app->enqueueMessage(JText::_('MOD_B4_CAROUSEL_NO_CATEGORY_OR_ITEMS'),'notice');
		return;
	}
} else {
	$slides = modB4CarouselHelper::getImagesFromFolder($params);
	if($slides==null) {
		$app->enqueueMessage(JText::_('MOD_B4_CAROUSEL_NO_CATALOG_OR_FILES'),'notice');
		return;
	}
}
...
if($theme != '_override') {
	$css = 'modules/mod_b4_carousel/themes/'.$theme.'/css/djimageslider.css';
} else {
	$theme = 'override';
...
$db = JFactory::getDBO();
$db->setQuery("SELECT manifest_cache FROM #__extensions WHERE element='mod_b4_carousel' LIMIT 1");
...
if ($jquery) {
	...
	$document->addScript(JURI::root(true).'/modules/mod_b4_carousel/assets/js/slider.js?v='.$ver, 'text/javascript', $canDefer);
} else {
	JHTML::_('behavior.framework', true);
	$document->addScript(JURI::root(true).'/modules/mod_b4_carousel/assets/js/moo.slider.js?v='.$ver, 'text/javascript', $canDefer);
}

if($params->get('link_image',1) > 1) {
	if($jquery) {

		$document->addScript(JURI::root(true).'/modules/mod_b4_carousel/assets/js/magnific-init.js', 'text/javascript', $canDefer);
	} else {
		$document->addScript(JURI::root(true).'/modules/mod_b4_carousel/assets/slimbox/js/slimbox.js', 'text/javascript', $canDefer);
		$document->addStyleSheet(JURI::root(true).'/modules/mod_b4_carousel/assets/slimbox/css/slimbox.css');
	}
}
...
$animationOptions = modB4CarouselHelper::getAnimationOptions($params);
...
$style = modB4CarouselHelper::getStyles($params);
$navigation = modB4CarouselHelper::getNavigation($params,$mid);
$show = (object) array('arr'=>$params->get('show_arrows'), 'btn'=>$params->get('show_buttons'), 'idx'=>$params->get('show_custom_nav'));

require JModuleHelper::getLayoutPath('mod_b4_carousel', $params->get('layout','default'));
